...
import org.locationtech.geomesa.accumulo.index.{Strategy, QueryPlanner, AttributeIdxStrategy, QueryStrategyDecider}
...
abstract class AccumuloFeatureWriterTest extends Specification with TestWithDataStore with BeforeExample {
  sequential
  ...
  lazy val queryPlanner = new QueryPlanner(sft, ds.getFeatureEncoding(sft), ds.getIndexSchemaFmt(sftName), ds,
    ds.strategyHints(sft))

  lazy val GeospatialStrategyOpt =
    if (spec.matches("(?i).*:geom:Point:.*")) Some(StrategyType.Z3)
    else Some(StrategyType.ST)
  lazy val AgeStrategyOpt =
    if (spec.matches("(?i).*age:Integer:index=true.*")) Some(StrategyType.ATTRIBUTE)
    else None
  lazy val NameStrategyOpt =
    if (spec.matches("(?i).*name:String:index=true.*")) Some(StrategyType.ATTRIBUTE)
    else None
  lazy val TemporalStrategyOpt =
    if (spec.matches("(?i).*dtg:Date:index=true.*")) Some(StrategyType.ATTRIBUTE)
    else None
  lazy val IdStrategyOpt = Some(StrategyType.RECORD)
  ...
  "AccumuloFeatureWriter" should {
    "provide ability to update a single feature that it wrote and preserve feature IDs" in {
      ...
      def queryViaPlanner(cqlFilter: Filter, strategyOpt: Option[StrategyType] = None) =
        queryPlanner.runQuery(new Query(sftName, cqlFilter), strategyOpt).toSeq

      def queryViaStore(cqlFilter: Filter) =
        fs.getFeatures(new Query(sftName, cqlFilter)).features().toSeq

      def doQuery(cqlFilter: Filter, shouldBeEmpty: Boolean = false, strategyOpt: Option[StrategyType] = None) = {
        println(s"[${getClass.getName} QUERY${if (shouldBeEmpty) " EMPTY" else ""} $strategyOpt] filter:  $cqlFilter")
...
class AccumuloFeatureWriterPointTest extends AccumuloFeatureWriterTest {
  val spec = "name:String:index=true,age:Integer,dtg:Date,geom:Point:srid=4326:index=full"
}


@RunWith(classOf[JUnitRunner])
class AccumuloFeatureWriterGeometryTest extends AccumuloFeatureWriterTest {
  val spec = "name:String:index=true,age:Integer,dtg:Date,geom:Geometry:srid=4326:index=full"
}


@RunWith(classOf[JUnitRunner])
class AccumuloFeatureWriterGeometryTestAgeIndex extends AccumuloFeatureWriterTest {
  val spec = "name:String:index=true,age:Integer:index=true,dtg:Date,geom:Geometry:srid=4326:index=full"
}


@RunWith(classOf[JUnitRunner])
class AccumuloFeatureWriterGeometryTestDateIndex extends AccumuloFeatureWriterTest {
  val spec = "name:String:index=true,age:Integer,dtg:Date:index=true,geom:Geometry:srid=4326:index=full"
}
