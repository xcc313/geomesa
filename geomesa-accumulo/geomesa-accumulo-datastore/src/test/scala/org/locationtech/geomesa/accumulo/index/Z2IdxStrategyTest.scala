...
import org.locationtech.sfcurve.zorder.{Z2, Z3}
...
class Z2IdxStrategyTest extends Specification with TestWithDataStore {
  ...
  override val spec = "name:String,*geom:Point:srid=4326"
  override val dtgField = None

  val features =
    (0 until 20).map { i =>
      val sf = new ScalaSimpleFeature(s"$i", sft)
      sf.setAttributes(Array[AnyRef](s"name$i", f"POINT(40 6${i % 10})"))
      sf
    } ++ (20 until 30).map { i =>
      val sf = new ScalaSimpleFeature(s"$i", sft)
      sf.setAttributes(Array[AnyRef](s"name$i", s"POINT(40 8${i - 20})"))
      sf
    }
  ...
  "Z3IdxStrategy" should {
    ...
    "return all features for inclusive filter" >> {
      ...
      ds.getQueryPlan(new Query(sftName, ECQL.toFilter(filter)), explainer = new ExplainPrintln)
      val features = execute(filter)
      features must haveSize(20)
      features.map(_.getID.toInt) must containTheSameElementsAs(0 until 20)
    }
