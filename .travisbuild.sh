...
mvn clean license:check install -Ptravis-ci -pl geomesa-cassandra/geomesa-cassandra-datastore -am 2>&1 | tee -a $BUILD_OUTPUT | grep -e '^\[INFO\] Building GeoMesa' -e '^\[INFO\] --- \(maven-surefire-plugin\|maven-install-plugin\|scala-maven-plugin.*:compile\)'
