...
/**
  * Updates filters to handle IDL issues and dwithin units, and to remove filters that aren't meaningful
  */
trait SafeTopologicalFilterVisitor extends DuplicatingFilterVisitorAccessor {
  ...
  override def visit(f: And, data: AnyRef): AnyRef = {
    ...
    if (children.exists(ignore)) {
      // ignore check after map to allow nested ignores to bubble up
      val newChildren = children.map(_.accept(this, data).asInstanceOf[Filter]).filterNot(ignore)
  ...
  private def ignore(f: Filter): Boolean = f == Filter.INCLUDE || isFilterWholeWorld(f)
}
